...
describe("verified occupancy merge", () => {
  it("merges a fresh verified record", () => {
    ...
    const [parking] = mergeVerifiedAvailability(
      [defaultParking],
      {
  ...
  it("rejects stale and negative occupancy", () => {
    ...
    const stale = mergeVerifiedAvailability(
      [defaultParking],
      {
        source: "operator",
        updatedAt: "2026-08-26T12:00:00Z",
        parkings: [{ id: defaultParking.id, spaces: 4 }],
      },
      now,
    )[0];
    const invalid = mergeVerifiedAvailability(
      [defaultParking],
      {
        source: "operator",
        updatedAt: "2026-08-26T12:59:00Z",
        parkings: [{ id: defaultParking.id, spaces: -1 }],
      },
      now,
    )[0];
