...
describe("parking radius results", () => {
  it("keeps every parking inside the selected walking radius", () => {
    ...
    const parkings = Array.from({ length: 30 }, (_, index) => ({
      ...defaultParking,
      id: `parking-${index}`,
      coordinates: { lat: 42.66 + index * 0.00001, lng: 21.16 },
      geometry: [
        [
          { lat: 42.66, lng: 21.16 },
          { lat: 42.66001, lng: 21.16 },
          { lat: 42.66, lng: 21.16001 },
        ],
      ],
    }));
    expect(walkableParkingCandidates(parkings, destination, 10)).toHaveLength(
      30,
    );
  });
});
