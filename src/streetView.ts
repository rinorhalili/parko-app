...
export function walkingDirectionsUrl(
  parking: Parking,
  destination: { coordinates: Parking["coordinates"] },
) {
  const origin =
    parking.geometry
      ?.flat()
      .reduce(
        (closest, point) =>
          distanceSquared(point, destination.coordinates) <
          distanceSquared(closest, destination.coordinates)
            ? point
            : closest,
        parking.accessPoint ?? parking.coordinates,
      ) ??
    parking.accessPoint ??
    parking.coordinates;
...
export function findCarDirectionsUrl(
  parking: Parking,
  parkedAt = parking.coordinates,
) {
  const params = new URLSearchParams({
    api: "1",
