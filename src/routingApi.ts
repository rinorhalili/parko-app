...
function instructionFor(step: OsrmStep) {
  ...
  if (
    ...
    return "Kthehu djathtas";
  if (
    modifier === "left" ||
    modifier === "slight left" ||
    modifier === "sharp left"
  )
    return "Kthehu majtas";
...
function walkingFallback(start: Coordinate, end: Coordinate): DrivingRoute {
  ...
  return {
    ...
    steps: [
      {
        instruction: "Ec drejt destinacionit",
        roadName: "Lidhje e përafërt",
        distanceMeters: distance,
        maneuverType: "walk",
      },
    ],
...
export async function loadDrivingMatrix(
  start: Coordinate,
  parkings: Parking[],
  signal?: AbortSignal,
): Promise<DrivingMatrixEntry[]> {
  if (!parkings.length) return [];
  const candidates = parkings.slice(0, 25);
  const coordinates = [
    start,
    ...candidates.map((parking) => parking.coordinates),
  ]
    .map(({ lng, lat }) => `${lng},${lat}`)
    .join(";");
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      return candidates.map((parking, index) => ({
        ...
        durationSeconds:
          payload.durations?.[0]?.[index] ?? parking.driveMinutes * 60,
        distanceMeters:
          payload.distances?.[0]?.[index] ?? parking.distanceMeters,
      }));
    } catch (error) {
      if (signal?.aborted) throw error;
    }
