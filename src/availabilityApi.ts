...
function statusFor(entry: AvailabilityEntry): Availability {
  ...
  if (entry.capacity && entry.spaces / entry.capacity <= 0.12) return "limited";
  return "available";
}

export function mergeVerifiedAvailability(
  parkings: Parking[],
  ...
  const entries = new Map(
    feed.parkings.map((entry) => [normalizedId(entry), entry]),
  );
  return parkings.map((parking) => {
    const entry = entries.get(parking.id);
    if (!entry || !Number.isFinite(entry.spaces) || entry.spaces < 0)
      return parking;
    ...
    const ageMinutes = Math.max(
      0,
      Math.round(
        (now - (Number.isFinite(updatedTime) ? updatedTime : feedUpdated)) /
          60_000,
      ),
    );
...
export async function loadVerifiedAvailability(
  ...
  if (!response.ok)
    throw new Error(`Occupancy feed returned ${response.status}`);
  const payload = (await response.json()) as Partial<AvailabilityFeed>;
  if (!payload.source || !payload.updatedAt || !Array.isArray(payload.parkings))
