...
export async function deliverPendingPushNotifications() {
  if (!env.EXPO_PUSH_ENABLED) return { processed: 0, delivered: 0 };

  const pending = await prisma.notification.findMany({
    where: { pushDeliveredAt: null, pushAttempts: { lt: 5 } },
    orderBy: { createdAt: "asc" },
    take: 100,
    include: { recipient: { select: { pushDevices: { where: { enabled: true }, select: { token: true } } } } }
  });
  ...
  for (const notification of pending) {
    ...
    try {
      ...
      logger.warn({ notificationId: notification.id, err: error }, "Expo push delivery failed");
    }
  }
