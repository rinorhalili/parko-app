...
export class AdminRepository {
  ...
  createParking(data: Prisma.ParkingSpotCreateInput) { return prisma.parkingSpot.create({ data }); }
  updateParking(id: string, data: Prisma.ParkingSpotUpdateInput) { return prisma.parkingSpot.update({ where: { id }, data }); }
  deleteParking(id: string) { return prisma.parkingSpot.delete({ where: { id } }); }
