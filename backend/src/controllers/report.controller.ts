...
import { createParkingReport, listParkingReports, voteOnParkingReport } from "../modules/reports/service.js";
...
export async function voteOnReport(req: Request, res: Response, next: NextFunction) { try { ok(res, await voteOnParkingReport(req.user!.id, req.params.id as string, req.body.vote)); } catch (error) { next(error); } }
